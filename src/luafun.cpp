...
int HAPI_GetDrawStrSize(lua_State* pL)
{
	const char* str = lua_tostring(pL, 1);
	int size = (int)lua_tonumber(pL, 2);
	const char* fontname = lua_tostring(pL, 3);
	int charset = (int)lua_tonumber(pL, 4);
	int OScharset = (int)lua_tonumber(pL, 5);

	int w = 0;
	int h = 0;
	JY_GetDrawStrSize(str, size, fontname, charset, OScharset, &w, &h);
	lua_pushnumber(pL, w);
	lua_pushnumber(pL, h);
